                ...
                if valueObject then
                    if typeof(value) == "number" then
                        if valueObject:IsA("NumberValue") then
                            ...
                            print("Replaced value of " .. name .. " in " .. path .. " with: " .. tostring(value))
                        else
                            warn(name .. " is not a NumberValue in " .. path)
                        end
                    else
                        if valueObject:IsA("StringValue") then
                            valueObject.Value = value
                            print("Replaced value of " .. name .. " in " .. path .. " with: " .. value)
                        else
                            warn(name .. " is not a StringValue in " .. path)
                        end
                    end
                else
...
local KillSoundType = 7293523910
------------------------------------ GUI STUFF ------------------------------------
...
MiscSec2:AddSlider('mov_bhopspeed', {Text = 'Bhop Speed', Default = 20, Min = 1, Max = 20, Rounding = 0, Compact = false})
...
MiscSec4:AddDropdown('hit_killsoundtype', {Values = {'Retro', 'UltraKill', 'Minecraft'}, Default = 1, Multi = false, Text = 'Kill Sound Type'})
Options.hit_hitsoundtype:OnChanged(function()
    if Options.hit_hitsoundtype.Value == "Retro" then
        KillSoundType = 7293523910
